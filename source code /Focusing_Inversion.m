function varargout = Focusing_Inversion(varargin)
% FOCUSING_INVERSION MATLAB code for Focusing_Inversion.fig
%      FOCUSING_INVERSION, by itself, creates a new FOCUSING_INVERSION or raises the existing
%      singleton*.
%
%      H = FOCUSING_INVERSION returns the handle to a new FOCUSING_INVERSION or the handle to
%      the existing singleton*.
%
%      FOCUSING_INVERSION('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in FOCUSING_INVERSION.M with the given input arguments.
%
%      FOCUSING_INVERSION('Property','Value',...) creates a new FOCUSING_INVERSION or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before Focusing_Inversion_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to Focusing_Inversion_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help Focusing_Inversion

% Last Modified by GUIDE v2.5 07-Apr-2023 09:23:02

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @Focusing_Inversion_OpeningFcn, ...
                   'gui_OutputFcn',  @Focusing_Inversion_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before Focusing_Inversion is made visible.
function Focusing_Inversion_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to Focusing_Inversion (see VARARGIN)

% Choose default command line output for Focusing_Inversion
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes Focusing_Inversion wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = Focusing_Inversion_OutputFcn(hObject, eventdata, handles) 
...
Obsgrid2D1D(:,1)=xyz(:,1);
Obsgrid2D1D(:,2)=xyz(:,2);
Obsgrid2D1D(:,3)=xyz(:,3);%observation height
...
ck2=get(handles.checkbox7,'value');
z0 = str2double(get(handles.edit_z0,'string'));
b = str2double(get(handles.edit_b,'string'));
%Generate Depth Weighing Matrix
if ck1==1
Z  = sparse(genWz( z0, paramGrid, b,zEnd ));
else
    Z=1;
end
...
A=ForwardMatrix;
 Wm=Z;
% Wm=diag(diag(A'*A).^(1/2)); 
dObs=xyz(:,4);

if ck2==0
Wd=eye(length(dObs));
else
    Wd=sparse(diag(1./xyz(:,5).^2));
end
...
ee=str2double(get(handles.edit_as,'string'));
%initialize the regularization parameter
[x, flag, relres, iter, resvec] = conjugateGradient0(A'*A,A'*dObs, 1e-21, 300);
m1=x;
mu=(A*m1-dObs)'*(A*m1-dObs)/((Wm*m1)'*(Wm*m1));
 
%use the focused results as prior information
% [m0] =Conjugradient2(A,dObs,Wd,Wm,ee,m0,Nmax,sigma,tolorence,m_low,m_max,mu);

[p] =Conjugradient3(A,dObs,Wd,Wm,ee,m0,Nmax,sigma,tolorence,m_low,m_max,mu);
xyzd=paramGrid;
xyzd(:,7)=p;
waitbar(1,h,'ok');
close(h);
